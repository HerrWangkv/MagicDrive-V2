...
from magicdrivedit.utils.train_utils import MaskGenerator, create_colossalai_plugin, update_ema, run_brushnet_validation, sp_vae
...
def main():
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        # Single GPU training - initialize distributed with single process
        os.environ["RANK"] = "0"
        ...
        dist.init_process_group(
            backend="nccl", rank=0, world_size=1, timeout=timedelta(hours=24)
        )
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                with timers["mask"] as mask_t:
                    ...
                    x_human = rearrange(x_human, "(B NC) C T ... -> B (C NC) T ...", NC=NC)  # B, (C, NC), T, H, W
                    human_mask = rearrange(human_mask, "(B NC) C T ... -> B (C NC) T ...", NC=NC)  # B, (C, NC), T, H, W
                    mask = None
                    if cfg.get("mask_ratios", None) is not None:
