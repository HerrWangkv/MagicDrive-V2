...
def main():
    ...
    with tqdm(
        ...
        for i, batch in pbar:
            ...
            # Load pedestrian video and mask from pedestrian_video_dir
            ...
            pedestrian_frames = pedestrian_frames[: int(model_args["num_frames"]), ...].to(device)
            ...
            # Pre-compute VAE encoding for PPD-style initialization
            with torch.no_grad():
                if cfg.sp_size > 1:
                    z0 = sp_vae(
                        pedestrian_frames,  # (B NC) C T ...
                        partial(vae.encode),
                        get_sequence_parallel_group(),
                    )
                else:
                    z0 = vae.encode(pedestrian_frames)
            ...
            for ns in range(num_sample):
                ...
                # PPD-style initialization
                z_flat = rearrange(z0, "B C T ... -> (B T) C ...")
                input_noise = torch.randn_like(z_flat)

                chunk_size = NC
                structured_noise_list = []
                cutoff_radius = cfg.get("cutoff_radius", None)
                print(f"Using cutoff_radius: {cutoff_radius}")
                for i in range(0, z_flat.shape[0], chunk_size):
                    ...
                    with torch.no_grad():
                        out_chunk = generate_structured_noise_batch_vectorized(
                            z_chunk,
                            cutoff_radius=cutoff_radius if cutoff_radius is not None else min(H, W)//2,
                ...
                if cfg.scheduler.type == "dpm-solver" and cfg.scheduler.cfg_scale == 1.0 or (
                    cfg.scheduler.type in ["rflow-slice", "rflow-ppd-slice"]
                ):
