...
from magicdrivedit.utils.train_utils import MaskGenerator, create_colossalai_plugin, update_ema, run_unconditioned_ppd_validation, sp_vae
...
def main():
    ...
    if cfg.get("validation_before_run", False):
        with RandomStateManager(verbose=True):
            coordinator.block_all()
            run_unconditioned_ppd_validation(
                cfg.val,
                text_encoder,
                vae,
                model,
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                with timers["move_data"] as move_data_t:
                    ...
                    x = torch.where(human_mask > 0.5, x, torch.ones_like(x)) # white background
                ...
                if report_every > 0 and (global_step + 1) % report_every == 0:
                    torch.cuda.synchronize()
                    torch.cuda.empty_cache()
                    val_dir = run_unconditioned_ppd_validation(
                        cfg.val,
                        text_encoder,
                        vae,
                        model,
