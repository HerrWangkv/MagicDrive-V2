...
def load_video_frames(video_path: Union[str, Path]) -> torch.Tensor:
    ...
    Returns:
        ...
        - RGB videos: shape (T, 3, H, W) with values in range [0, 1]
        - Grayscale videos: shape (T, 1, H, W) with values in range [0, 1]
    ...
    video_tensor, _, _ = read_video(str(video_path), pts_unit="sec")
...
def compress_time_for_mask(mask, factor=4):
    """
    Compress the time dimension of a mask by a given factor according to CogVAE's strategy.
    Args:
        mask (torch.Tensor): Input mask tensor of shape (B, 1, T, H, W)
        factor (int): Factor by which to compress the time dimension

    Returns:
        torch.Tensor: Compressed mask tensor of shape (B, 1, T//factor, H, W)
    """
    B, _, T, H, W = mask.shape
    level = np.log2(factor).astype(int)

    def compress_as_in_CogVideoXCausalConv3d(mask):
        # CogVideoXCausalConv3d: just padding, no compression
        maskp1 = torch.roll(mask, shifts=1, dims=2)
        maskp1[:, :, 0] = mask[:, :, 0]  # first frame
        maskp2 = torch.roll(maskp1, shifts=1, dims=2)
        maskp2[:, :, 0] = mask[:, :, 0]  # first frames
        return (mask + maskp1 + maskp2) / 3

    def compress_as_in_CogVideoXDownsample3d(mask):
        # CogVideoXDownsample3D: compress by 2 without overlap
        mask = rearrange(mask, "B C T H W -> (B H W) C T")
        if mask.shape[-1] % 2 == 1:
            mask_first, mask_rest = mask[:, :, 0], mask[:, :, 1:]
            if mask_rest.shape[-1] > 0:
                mask_rest = F.avg_pool1d(mask_rest, kernel_size=2, stride=2)
            mask = torch.cat([mask_first[..., None], mask_rest], dim=-1)
        else:
            mask = F.avg_pool1d(mask, kernel_size=2, stride=2)
        mask = rearrange(mask, "(B H W) C T -> B C T H W", H=H, W=W)
        return mask

    mask = compress_as_in_CogVideoXCausalConv3d(mask)
    for _ in range(level):
        mask = compress_as_in_CogVideoXDownsample3d(mask)
    mask = compress_as_in_CogVideoXCausalConv3d(mask)
    return mask
...
def main():
    ...
    # == init exp_dir ==
    cfg.outputs = cfg.get("outputs", "outputs/test_repaint")
    ...
    with tqdm(
        ...
        for i, batch in pbar:
            ...
            for ns in range(num_sample):
                ...
                if 8 in model_args["bbox"]["classes"]:
                    pedestrian_exists = True
                    assert B == 1, "Only support bs=1 for pedestrian repainting"
                    pedestrian_video_dir = "data/val_videos"  # TODO: config this
                    ...
                    HEIGHT, WIDTH = pedestrian_frames.shape[-2:]
                    h = HEIGHT // 2
                    w = WIDTH // 3
                    assert (
                    ...
                    pedestrian_mask_frames = torch.concat(
                        [
                            pedestrian_mask_frames[:, :, :h, :],
                            pedestrian_mask_frames[:, :, h:, :],
                        ],
                        dim=3,
                    )
                ...
                if pedestrian_exists:
                    ...
                    pedestrian_mask_frames = rearrange(
                        pedestrian_mask_frames.expand(-1, vae.out_channels, -1, -1, -1),
                        "(B NC) C T ... -> B (C NC) T  ...",
                        NC=NC,
                    )
                    ...
                    # mask_pedestrian = F.interpolate(
                    #     pedestrian_mask_frames.to(dtype),
                    #     size=z_pedestrian.shape[-2:],
                    #     mode="area",
                    # )
                    # mask_pedestrian = rearrange(
                    #     mask_pedestrian,
                    #     "B (C NC T) H W -> B (C NC) T H W",
                    #     NC=NC,
                    #     T=int(model_args["num_frames"]),
                    # )
                    # mask_pedestrian = compress_time_for_mask(mask_pedestrian, factor=4)
                ...
                if (
                    cfg.scheduler.type == "dpm-solver"
                    and cfg.scheduler.cfg_scale == 1.0
                    or (cfg.scheduler.type in ["rflow-slice", "rflow-slice-repaint"])
                ):
                ...
                samples = scheduler.sample(
                    ...
                    progress=verbose >= 1,
                    mask=mask_pedestrian if pedestrian_exists else None,
                )
