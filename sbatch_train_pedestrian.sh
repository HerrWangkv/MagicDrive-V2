#!/bin/bash
#SBATCH --job-name=magicdrive_pedestrian
#SBATCH --nodes=8
...
#SBATCH --output=logs/train_pedestrian_%j.out
#SBATCH --error=logs/train_pedestrian_%j.err
...
srun --label --export=ALL --ntasks-per-node=1 --gres=gpu:4 \
    ...
    bash -c "
        ...
        torchrun --nproc-per-node=4 --nnodes=\$SLURM_JOB_NUM_NODES --node_rank=\$NODE_RANK \
          ...
          scripts/train_pedestrian.py configs/magicdrive/train/pedestrian.py \
          --cfg-options num_workers=1 prefetch_factor=2"
