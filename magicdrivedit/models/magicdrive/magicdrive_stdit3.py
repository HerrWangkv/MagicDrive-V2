...
class MagicDriveSTDiT3SDEBrushNet(MagicDriveSTDiT3BrushNet):
    """
    Extended BrushNet that supports independent timestep for inpaint branch.
    This allows SDE-style noise scheduling where x_inpaint can have different noise levels.
    """
    ...
    def __init__(self, config: MagicDriveSTDiT3Config):
        ...
        # Add separate timestep embedding block for inpaint branch
        self.t_inpaint_block = nn.Sequential(
            nn.SiLU(),
            nn.Linear(self.hidden_size, 6 * self.hidden_size, bias=True)
        )
        self.t_combine_block = nn.Sequential(
            nn.SiLU(),
            nn.Linear(12 * self.hidden_size, 6 * self.hidden_size, bias=True)
        )
        ...
        if config.only_train_extra_blocks:
            ...
            for param in self.shallow_encoder.parameters():
                param.requires_grad = True
            for param in self.t_inpaint_block.parameters():
                param.requires_grad = True
            for param in self.t_combine_block.parameters():
                param.requires_grad = True
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward pass of MagicDrive SDE BrushNet.
        
        Args:
            timestep_inpaint: Independent timestep for x_inpaint branch.
        """
        dtype = self.x_embedder.proj.weight.dtype
        B, real_T = x.size(0), rel_pos.size(1)
        if drop_cond_mask is None:  # camera
        ...
        # === get independent timestep embed for inpaint using separate t_inpaint_block ===
        t_inpaint = self.t_embedder(timestep_inpaint, dtype=x.dtype)  # [B, C]
        t_inpaint = t_inpaint + fps
        t_inpaint_mlp = self.t_inpaint_block(t_inpaint)  # Use separate block for inpaint
        ...
        if x_mask is not None:
            ...
            t0 = t0 + fps
            t0_mlp = self.t_block(t0)
            # Also create t0 for inpaint using separate block
        ...
        for block_i in range(0, self.control_depth):
            ...
            # Use independent timestep for inpaint branch
            x_inpaint, x_inpaint_skip = auto_grad_checkpoint(
                self.brushnet_blocks_s[block_i],
                x_inpaint,
                y,
                t_combined_mlp,  # Use t_combined_mlp instead of t_mlp
                y_lens,
                x_mask,
                t0_combined_mlp,
                T,
...
@MODELS.register_module("MagicDriveSTDiT3-XL/2-SDEBrushNet")
def MagicDriveSTDiT3_XL_2_SDEBrushNet(
    from_pretrained=None, force_huggingface=False, **kwargs
):
    if from_pretrained is not None and not (os.path.exists(from_pretrained)):
        model = MagicDriveSTDiT3SDEBrushNet.from_pretrained(from_pretrained, **kwargs)
    else:
        from_pretrained_pixart = kwargs.pop("from_pretrained_pixart", None)
        config = MagicDriveSTDiT3Config(
            depth=28, hidden_size=1152, patch_size=(1, 2, 2), num_heads=16, **kwargs
        )
        model = MagicDriveSTDiT3SDEBrushNet(config)
        if (
            from_pretrained is not None and force_huggingface
        ):  # load from hf stdit3 model
            load_from_stdit3_pretrained(model, from_pretrained)
